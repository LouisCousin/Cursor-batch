...
class CorpusManager:
    ...
    def from_dataframe(cls, df: pd.DataFrame, keywords_json_path: Optional[str] = None, column_overrides: Optional[dict] = None):
        """Crée un CorpusManager à partir d'un DataFrame déjà chargé"""
        instance = cls.__new__(cls)
        instance.path = None
        instance.kmap_path = keywords_json_path
        instance.df = df
        instance.kmap = instance._load_keywords(keywords_json_path) if keywords_json_path else {}
        instance.overrides = column_overrides or {}
        instance._setup_columns()
        return instance
    
    def _setup_columns(self):
        """Configure les colonnes du DataFrame"""
